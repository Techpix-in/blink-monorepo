...
export class GroupMembershipRepository extends BaseRedisRepository {
    ...
    async joinGroup(groupId: string, connectionId: string): Promise<void> {
        try {
            ...
            const groupMembershipKey = this.getKey('group', `${groupId}:members`);
            await client.sAdd(groupMembershipKey, connectionId);
        } catch (error) {
    ...
    async leaveGroup(groupId: string, connectionId: string): Promise<void> {
        try {
            const client = this.getClient();
            const groupMembershipKey = this.getKey('group', `${groupId}:members`);
            await client.sRem(groupMembershipKey, connectionId);
        } catch (error) {
